...
module Jekyll
    class LoopDirectoryTag < Liquid::Block
        ...
        def filter_out_thumbnails(images)
            images.find_all{|e| not e.end_with?("thumbnail.jpg")}
        end

        def render(context)
            context.registers[:loop_directory] ||= Hash.new(0)

            name = Pathname.new(context.registers[:page]["id"]).basename
            images = Dir.glob(File.join(@attributes['directory'], name, @attributes['filter']))

            images = filter_out_thumbnails(images)
